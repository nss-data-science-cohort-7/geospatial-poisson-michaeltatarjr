...
```
So just so we're clear here, this means there are 40 tracts with 1 incident report, and 24 tracts with 2...1 tract with 39.
```{r}
burglaries_joined|> 
...
ggplot(burglaries_joined, aes( x=incident_number, y=poplog, hue=group)) +
  geom_point(color='darkblue') 
```
