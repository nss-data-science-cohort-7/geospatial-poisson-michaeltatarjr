...
burglaries_combined |> 
  group_by(TRACTCE) |> 
  distinct(incident_number) |> 
  count(name = "incident_numbers") |> 
  mutate(incidents_by_pop = "incident_numbers"/'population') |> 
  arrange(desc(incident_numbers))

```

The above error means that I am trying to divide between a numeric and non numeric operator.
  #check for class in pop column, shows it is numeric...
  #it looks like it is the count column.  Can we recast this or add a column into the df?
```{r}
class(burglaries_combined$popuation)
```
